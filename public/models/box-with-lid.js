...
export default function main({
  ...
  const innerTray = drawRoundedRectangle(width, height)
    .sketchOnPlane("XY")
  ...
  const { walls, lid } = hingedWalls({
    width,
    height,
    depth: depth + bottomThickness + tolerance,
    wallThickness,
  ...
  const box = walls
    .fuse(innerTray)
    .fillet(1, e => e.inPlane("XY", bottomThickness));

  if (!printInPlace) {
    return [
      { shape: box.translateX(width / 2 + 5), name: "Box" },
      { shape: lid.translateX(-width / 2 - 5), name: "Lid" },
    ];
  }
