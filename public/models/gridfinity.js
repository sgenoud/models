...
const buildTopShape = ({
  ...
  const topShape = (basePlane, startPosition) => {
    const withLip = includeLip && wallThickness <= SOCKET_TAPER_WIDTH;
    ...
    const shiftedShape = intersectBlueprints(
      ...
      roundedRectangleBlueprint(10, 10).translate(-5, withLip ? 0 : 5)
    );
    ...
    if (includeLip) {
      ...
      topProfile = cutBlueprints(
        topProfile,
        roundedRectangleBlueprint(wallThickness, 10).translate(
          -wallThickness / 2,
          -5
        )
      );
